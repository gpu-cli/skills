#!/usr/bin/env bash
# logic: Claude Code SessionStart hook (Layer-2 orientation, not enforcement).
#
...
EOF
  ...
  msg="Decision-trail tracking is ON for branch '${logical}' (storage: ${storage}). Log decision points as you work — a fork chosen, a unit finished with its check, a pivot or revert — with /logic track \"<what> because <why>\" --confidence <high|medium|low> or the logic log helper. Confidence is your perception, not a correctness score; commit stubs and unassessed legacy rows remain unknown. Commit stubs are captured automatically; give the ones that mattered a one-line 'why' and confidence before finishing. The row protocol lives in the logic track reference."

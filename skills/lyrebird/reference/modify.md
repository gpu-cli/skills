...
If the adapted post is written to a file, run:

```bash
node {{scripts_path}}/validate-social-output.mjs --mode modify --file <path> --platform <platform>
```

When the original post is a local file, pass it too so the validator can confirm image metadata was preserved rather than newly added:

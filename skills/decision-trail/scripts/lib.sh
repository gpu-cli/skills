#!/usr/bin/env bash
# decision-trail engine library.
#
# Sourced by every decision-trail helper (interactive skill scripts) and, in its
# materialized form at .decision-trail/runtime/decision-trail.sh, by the git post-commit hook and
...
# The runtime directory. A repo enabled before the skill was renamed still has
# .logic/; read it where it is rather than reporting the branch as untracked.
# migrate-legacy.sh moves it, and install-hooks.sh runs that on every toggle on.
dt_dir() {
  local root; root=$(dt_repo_root) || return 1
  [ -z "$root" ] && return 1
  if [ ! -d "$root/.decision-trail" ] && [ -f "$root/.logic/config.json" ]; then
    printf '%s/.logic' "$root"
    return
  fi
  printf '%s/.decision-trail' "$root"
}

dt_config_path() {
  printf '%s/config.json' "$(dt_dir)"
}
...
dt_actor() {
  if [ -n "${DECISION_TRAIL_ACTOR:-}" ]; then printf '%s' "$DECISION_TRAIL_ACTOR"; return; fi
...
dt_resolve_ref() {
  local b="${1:-}"
  if [ -z "$b" ] || [ "$b" = "$(dt_current_branch)" ]; then printf 'HEAD'; return 0; fi
...
dt_bd_capability() {
  dt_have bd || { printf 'missing'; return 0; }
  local dir cache key ver ck cv verdict
  dir="$(dt_dir 2>/dev/null)"
...
dt_bd_ok() { [ "$(dt_bd_capability)" = "ok" ]; }

# --- config ---------------------------------------------------------------

# Read config.json; prints "{}" if missing or jq unavailable.
dt_config_raw() {
  local p; p=$(dt_config_path)
  if [ -f "$p" ] && dt_have jq; then cat "$p"; else printf '{}'; fi
}

dt_config_get() {
  # dt_config_get <jq-filter> [default]
  local filter="$1" def="${2:-}"
  if ! dt_have jq; then printf '%s' "$def"; return; fi
  local v; v=$(dt_config_raw | jq -r "$filter // empty" 2>/dev/null)
...
dt_configured_storage() {
  local logical="${1:-}" s
  s=$(dt_config_get ".storageOverrides[\"$logical\"]" "")
  [ -z "$s" ] && s=$(dt_config_get '.storage' 'beads')
  printf '%s' "$s"
}
...
dt_effective_state() {
  local branch; branch=$(dt_current_branch)
  local default storage state logical
  default=$(dt_config_get '.default' 'off')
  storage=$(dt_config_get '.storage' 'beads')
  state="$default"; logical="$branch"

  if dt_have jq && [ -f "$(dt_config_path)" ]; then
    local raw; raw=$(dt_config_raw)
    # 1. exact match
    ...
    if [ -n "$exact" ]; then
      ...
      while IFS=$'\t' read -r key val; do
        [ -z "$key" ] && continue
        if dt_glob_match "$branch" "$key"; then matched="$key"; state="$val"; logical="$key"; break; fi
  ...
  if [ "$storage" = "beads" ] && ! dt_bd_ok; then storage="tsv"; fi
  printf '%s %s %s' "$state" "$logical" "$storage"
}

dt_is_tracked() {
  local s; s=$(dt_effective_state); [ "${s%% *}" = "on" ]
}
...
# deduped .decision-trail/WARNINGS file for hooks (surfaced by the SessionStart hook).

# Prints the reason the beads backend is unusable; returns 1 when it is fine.
dt_backend_note() {
  local cap; cap="$(dt_bd_capability)"
  case "$cap" in
    ok) return 1 ;;
    missing)
      printf 'decision-trail: bd is not on PATH — decision rows are going to the TSV fallback at .decision-trail/audit/.' ;;
    *)
      printf 'decision-trail: the bd on PATH (%s) lacks the API this suite needs (bd query, --metadata, decision type) — rows are going to the TSV fallback at .decision-trail/audit/. If bd works in your interactive shell, you likely have more than one install and a login shell resolves a different one.' \
...
dt_downgraded() {
  ...
  es=$(dt_effective_state)
  logical=$(printf '%s' "$es" | awk '{print $2}')
  storage=$(printf '%s' "$es" | awk '{print $3}')
  [ "$storage" = "tsv" ] || return 1
  [ "$(dt_configured_storage "$logical")" = "beads" ]
}

# Append a note to .decision-trail/WARNINGS, deduped. Safe inside hooks.
dt_record_note() {
  local t="${1:-}" d f
  [ -z "$t" ] && return 0
  d="$(dt_dir 2>/dev/null)" || return 0
  [ -z "$d" ] && return 0
...
dt_record_warning() {
  dt_downgraded || return 0
  dt_record_note "$(dt_backend_note)"
}

# Interactive one-shot warning to stderr.
dt_warn_stderr() {
  dt_downgraded || return 0
  local t; t="$(dt_backend_note)"
  [ -n "$t" ] && printf '%s\n' "$t" >&2
  return 0
}
...
DECISION_TRAIL_TSV_HEADER='ts	actor	phase	decision	why	evidence	result	kind	sha	worktree	confidence'

# Coarse by design: confidence is the actor's assessment, not a correctness score.
dt_normalize_confidence() {
  case "${1:-unknown}" in
...
# _dt_log_beads <kind> <actor> <phase> <decision> <why> <evidence> <result> <sha> <confidence> <slug> <logical> <wt> <ts>
_dt_log_beads() {
  local kind="$1" actor="$2" phase="$3" decision="$4" why="$5" evidence="$6" result="$7" sha="$8"
  local confidence="$9" slug="${10}" logical="${11}" wt="${12}" ts="${13}"
  local labels="decision-trail:${slug}"
  [ -z "$why" ] && labels="${labels},decision-trail-stub"
  local meta id
  if dt_have jq; then
    meta=$(jq -nc \
...
# _dt_log_tsv <kind> <actor> <phase> <decision> <why> <evidence> <result> <sha> <confidence> <slug> <wt> <ts>
_dt_log_tsv() {
  local kind="$1" actor="$2" phase="$3" decision="$4" why="$5" evidence="$6" result="$7" sha="$8"
  local confidence="$9" slug="${10}" wt="${11}" ts="${12}"
  local dir; dir="$(dt_dir)/audit/${slug}"
  mkdir -p "$dir" 2>/dev/null
  local file
  file="${dir}/$(dt_slug "$actor").tsv"
  if [ ! -f "$file" ]; then printf '%s\n' "$DECISION_TRAIL_TSV_HEADER" >"$file"; fi
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$(dt_sanitize_cell "$ts")" \
    "$(dt_sanitize_cell "$actor")" \
    "$(dt_sanitize_cell "$phase")" \
    "$(dt_sanitize_cell "$decision")" \
    "$(dt_sanitize_cell "$why")" \
    "$(dt_sanitize_cell "$evidence")" \
    "$(dt_sanitize_cell "$result")" \
    "$(dt_sanitize_cell "$kind")" \
    "$(dt_sanitize_cell "$sha")" \
    "$(dt_sanitize_cell "$wt")" \
    "$(dt_sanitize_cell "$confidence")" \
    >>"$file"
  printf '%s' "$file"
}

# dt_log_row <kind> <actor> <phase> <decision> <why> <evidence> <result> <sha> [confidence]
...
dt_log_row() {
  local kind="$1" actor="$2" phase="$3" decision="$4" why="$5" evidence="$6" result="$7" sha="$8"
  local confidence; confidence=$(dt_normalize_confidence "${9:-unknown}")
  local es logical storage
  es=$(dt_effective_state)
  logical=$(printf '%s' "$es" | awk '{print $2}')
  storage=$(printf '%s' "$es" | awk '{print $3}')
  local slug; slug=$(dt_slug "$logical")
  local wt; wt=$(dt_repo_root)
  local ts; ts=$(dt_now)

  if [ "$storage" = "beads" ]; then
    local id
    id=$(_dt_log_beads "$kind" "$actor" "$phase" "$decision" "$why" "$evidence" "$result" "$sha" "$confidence" "$slug" "$logical" "$wt" "$ts")
    if [ -n "$id" ]; then printf '%s' "$id"; return 0; fi
    dt_record_note "decision-trail: a bd write failed; that decision row went to the TSV fallback at .decision-trail/audit/ instead."
  else
    dt_record_warning
  fi
...
dt_find_stub_by_sha() {
  ...
  logical=$(printf '%s' "$es" | awk '{print $2}')
  storage=$(printf '%s' "$es" | awk '{print $3}')
  slug=$(dt_slug "$logical")

  if [ "$storage" = "beads" ] && dt_bd_ok && dt_have jq; then
    bd query "label=decision-trail:${slug}" --all --json 2>/dev/null \
      | jq -r --arg s "$sha" '.[] | select((.metadata.sha // "")==$s) | .id' 2>/dev/null \
      | head -1
    return 0
  fi
  dt_tsv_row_for_sha "$sha" "$slug"
}
...
# beads: update the bead in place and drop the decision-trail-stub label.
...
# dt_enrich <bead-id-or-sha> <why> [result] [confidence]
dt_enrich() {
  local ref="$1" why="$2" result="${3:-}" confidence="${4:-}"
  [ -n "$confidence" ] && confidence=$(dt_normalize_confidence "$confidence")
  local es logical storage slug
  es=$(dt_effective_state)
  logical=$(printf '%s' "$es" | awk '{print $2}')
  storage=$(printf '%s' "$es" | awk '{print $3}')
  ...
  if [ "$storage" = "beads" ] && dt_bd_ok; then
    local id="$ref" resolved
    # allow enriching by SHA
    if dt_have jq; then
      resolved=$(bd query "label=decision-trail:${slug}" --all --json 2>/dev/null \
    ...
    bd label remove "$id" decision-trail-stub >/dev/null 2>&1
    if { [ -n "$result" ] || [ -n "$confidence" ]; } && dt_have jq; then
      local cur newmeta
  ...
  line=$(dt_tsv_row_for_sha "$ref" "$slug") || return 1
  ...
  actor=$(dt_actor)
  _dt_log_tsv "enrich" "$actor" "$phase" "$decision" "$why" "$evidence" "$result" "$ref" "$confidence" \
    "$slug" "$(dt_repo_root)" "$(dt_now)"
}
